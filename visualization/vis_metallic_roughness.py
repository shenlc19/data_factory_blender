...
def vis_single(exr_file, save_path):
    ...
    cv2.imwrite(save_path, uint8_data)
...
if __name__ == "__main__":
    ...
    all_maps = []
    roughness_maps = natsort.natsorted(glob.glob(os.path.join(base_dir, 'Roughness/*')))
    roughness_save_dir = os.path.join(base_dir, 'Roughness_vis')
    os.makedirs(roughness_save_dir, exist_ok=True)
    for roughness_map_path in roughness_maps:
        vis_single(roughness_map_path, os.path.join(
            roughness_save_dir, os.path.basename(roughness_map_path).replace('.exr', '.png')
        ))
    metallic_maps = natsort.natsorted(glob.glob(os.path.join(base_dir, 'Metallic/*')))
    metallic_save_dir = os.path.join(base_dir, 'Metallic_vis')
    os.makedirs(metallic_save_dir, exist_ok=True)
    for metallic_map_path in metallic_maps:
        vis_single(metallic_map_path, os.path.join(
            metallic_save_dir, os.path.basename(metallic_map_path).replace('.exr', '.png')
        ))
