...
def main():
    ...
    # preferred = [os.path.join('/mnt/mydrive/shenlc/ArchDiff/rendering_single_view/', f'{i}/000.png') 
    #              for i in [16, 58, 437, 442, 715, 790, 864, 1105, 1668, 1866]]
    ...
    exclude_list = set(
        [1, 12] + list(range(338, 356)) + list(range(378, 397)) + list(range(553, 571)) + [1149, ] + list(range(1516, 1527))  + list(range(1757, 1773))
    )

    # files = [x for x in files if x not in preferred]
    # files = natsort.natsorted(glob.glob('/mnt/mydrive/shenlc/ArchDiff/rendering_single_view/*/000.png'))#[:250]
    files = [x for x in natsort.natsorted(glob.glob('datasets/tmp512/renders/*/035.png'))][:200]
