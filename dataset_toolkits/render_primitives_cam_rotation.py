...
def _render(file_path, sha256, output_dir, num_views, normal_map=False):
    ...
    args = [
        BLENDER_PATH, '-b', '-P', os.path.join(os.path.dirname(__file__), 'blender_script', 'render_cam_rotation_same_env_map.py'), # 'render_hi3dgen_pbr.py'),
        '--',
...
if __name__ == '__main__':
    ...
    _render(file_path=file_path, 
            sha256 = None, 
            output_dir=f"datasets/glassverse_reflective_table/{scene_name}",
            num_views=10,
            normal_map=True
            )
